...
bool eagle_stratum_set_target(struct stratum_ctx *sctx, json_t *params)
{
	const char *target_hex = json_string_value(json_array_get(params, 0));
	if (!target_hex || strlen(target_hex) == 0)
		return false;
	
	hex2bin(sctx->job.extra, target_hex, 32);
...
bool eagle_stratum_notify(struct stratum_ctx *sctx, json_t *params)
{
	const char *job_id, *header_hash, *parent_hash;
	uint32_t height;
	bool clean, ret = false;
	int p=0;
	job_id = json_string_value(json_array_get(params, p++));
	header_hash = json_string_value(json_array_get(params, p++));
	height = (uint32_t)json_integer_value(json_array_get(params, p++));
	parent_hash = json_string_value(json_array_get(params, p++));
	clean = json_is_true(json_array_get(params, p));
	
	pthread_mutex_lock(&stratum_work_lock);

	hex2bin(sctx->job.prevhash, header_hash, 32);
