...
start:
	...
	if (retry)
		...
		sprintf(s, "{\"id\": 1, \"method\": \"mining.subscribe\", \"params\": [\"" USER_AGENT "\", null]}");
...
static bool stratum_notify(struct stratum_ctx *sctx, json_t *params)
{
	...
	if (sctx->is_equihash) {
		...
		return eagle_stratum_notify(sctx, params);
	}
...
bool stratum_handle_method(struct stratum_ctx *sctx, const char *s)
{
	...
	if (!strcasecmp(method, "mining.set_target")) {
		...
		ret = eagle_stratum_set_target(sctx, params);
		goto out;
	}
