...
static bool eagle_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	int i;
	
	pthread_mutex_lock(&stratum_work_lock);
	snprintf(work->job_id, sizeof(work->job_id), "%07x %s", 0xfffffff, sctx->job.job_id);
	...
	memcpy(&work->data[0], sctx->xnonce1, sctx->xnonce1_size);
	for (i=0; i<8; i++)
		work->data[2 + i] = le32dec((uint32_t *)sctx->job.prevhash+i);

	/*for (i=0; i<40; i++)
		printf("%02x", *((uint8_t *)work->data+i));	
	printf("\n");*/

	pthread_mutex_unlock(&stratum_work_lock);
	memcpy(work->target, sctx->job.extra, 32);
	/*for (i=0; i<32; i++)
		printf("%02x", *((uint8_t *)work->target+i));	
	printf("\n");*/
...
static bool stratum_gen_work(struct stratum_ctx *sctx, struct work *work)
{
	...
	if (sctx->is_equihash){
		return eagle_gen_work(sctx, work);
	}
